...
HOUR=$(date +%H)
if [ $HOUR -lt 12 ]; then
    GREETING="Good Morning, $USER_NAME"#!/bin/bash
...
# --- 5. Net Speed (Aligned to Disk Bar) ---
...
if [ -n "$INTERFACE" ]; then
    ...
    sleep 0.5 # Snapshot duration
    
    # Get bytes after half a second
    R2=$(cat /sys/class/net/$INTERFACE/statistics/rx_bytes)
    T2=$(cat /sys/class/net/$INTERFACE/statistics/tx_bytes)
    
    # Calculate MB/s with 1 decimal point using awk
    RX=$(echo "scale=2; ($R2 - $R1) / 524288" | bc) 
    TX=$(echo "scale=2; ($T2 - $T1) / 524288" | bc)
    
    printf " ${WHITE}%-11s${RESET} ↓ %-10s | ↑ %-10s (via %s)\n" "NET SPEED:" "${RX} MB/s" "${TX} MB/s" "$INTERFACE"
fi
